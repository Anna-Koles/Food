...
function calc() {
  ...
  function calcTotal() {
    if (!sex || !height || !weight || !age || !ratio) {
      ...
      return; // чтобы функция не пошла дальше, если что-то не заполнено
    }
  ...
  calcTotal(); // чтобы польз-ль сразу видел надпись, что ввел не все данные
...
function slider(_ref) {
  ...
  var slides = document.querySelectorAll(slide),
      ...
      // только для моего варианта простого
  // это были для простого, для слайдера-карусели дополнительно нужны:
  ...
  var slideIndex = 1;
  var offset = 0; // простой (мой вариант)
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  tabsParent.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      // slice - чтобы убрать точку, т.к. здесь надо искать только по названию
